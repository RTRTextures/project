...
void main(void)
{
    if(gl_InvocationID == 0)
    {
        ...
        {
            float a = length(p2.xy - p0.xy) * 16.0 + 1.0;
            float b = length(p3.xy - p2.xy) * 16.0 + 1.0;
            float c = length(p3.xy - p1.xy) * 16.0 + 1.0;
            float d = length(p1.xy - p0.xy) * 16.0 + 1.0;

            gl_TessLevelOuter[0] = a;
            gl_TessLevelOuter[1] = b;
            gl_TessLevelOuter[2] = c;
            gl_TessLevelOuter[3] = d;
            gl_TessLevelInner[0] = min(b, d);
            gl_TessLevelInner[1] = min(a, c);
        }
    }
