...
namespace VSIUtil
{
	...
	VSIUtilPlus::~VSIUtilPlus()
	{
		if (szAppName[0] != L'\0')
		{
			UnregisterClass(szAppName, (HINSTANCE)GetModuleHandle(NULL));
			memset(szAppName, 0, sizeof(szAppName));
		}
	}
	void VSIUtilPlus::WindowInit()
	{
		wcscpy_s(szAppName, wcslen(TEXT("Default OpenGL Window")) + 1, TEXT("Default OpenGL Window"));
	}
	...
	void VSIUtilPlus::MessageHandler(UINT iMsg, WPARAM wParam, LPARAM lParam)
	{
